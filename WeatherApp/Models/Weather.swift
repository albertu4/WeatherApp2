...
struct Weather: Decodable {
    ...
    init(weatherData: [String: Any]) {
        ...
        let conditions = weatherData["currentConditions"] as? [String: Any] ?? [:]
        currentConditions = CurrentConditions.getCurrentWeather(from: conditions)
        
        let days = weatherData["next_days"] as? [[String: Any]] ?? [[:]]
        nextDays = NextDays.getForecast(from: days)
    }
